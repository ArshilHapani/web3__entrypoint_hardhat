...
!developmentChains.includes(network.name)
  ...
  : describe.skip("FundMe", function () {
      ...
      let fundMe: FundMe;
      const parameter =
        netWorkConfig[network.config.chainId ?? 1]?.ethUsdPriceFeed ?? "";
      ...
      describe.skip("fund", function () {
        ...
        it("updates the FundMe balance", async function () {
          ...
          const response = await fundMe.s_fundersToAmount(ownerAddress);
        ...
        it("Adds funder to the funders array", async function () {
          ...
          const funders = await fundMe.s_funders(0);
