...
import { deployDynamicSVGNFT, deployMocks } from "../scripts/dynamicNFTDeploy";
...
!developmentChains.includes(network.name)
  ...
  : describe("Dynamic NFT", () => {
      ...
      beforeEach(async function () {
        deployer = await getSigner().then((signer) => signer?.address ?? "");
        mockV3Aggregator = await deployMocks();
        dynamicSVGNFT = await deployDynamicSVGNFT(
      ...
      describe("constructor", () => {
        it("should sets starting values correctly", async () => {
          const loadSvg = await dynamicSVGNFT.getLowSvg();
          const highSvg = await dynamicSVGNFT.getHighSvg();
          const priceFeed = await dynamicSVGNFT.getPriceFeed();
          assert.equal(lowSVGImageuri, loadSvg);
          assert.equal(highSVGimageUri, highSvg);
          assert.equal(await mockV3Aggregator.getAddress(), priceFeed);
        });
      });

      describe.skip("mintNft", () => {
        it("emits an event and creates the NFT", async function () {
          ...
          await expect(dynamicSVGNFT.mintNFT(highValue))
            .to.emit(dynamicSVGNFT, "NewEpicNFTMinted")
        ...
        it.skip("shifts the token uri to lower when the price doesn't surpass the highvalue", async function () {
          const highValue = ethers.parseEther("1000000");
